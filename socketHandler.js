...
export default (io, socket) => {
  ...
  socket.on('user-details', ({ gender, interest, name, mode, selectedGender, interests }) => {
    try {
      ...
      socket.data = { 
        ...
        name, 
        mode,
        interests: interests || [], // Store interests array
        joinTime: Date.now(),
        isActive: true
      };
  ...
  function findBestMatch(socket, waitingUsers) {
    ...
    for (let [id, otherSocket] of waitingUsers) {
      ...
      let interestsMatchScore = 0;
      let hasInterestsMatch = false;
      
      if (socket.data.interests && otherSocket.data.interests && 
          socket.data.interests.length > 0 && otherSocket.data.interests.length > 0) {
        
        const commonInterests = socket.data.interests.filter(interest => 
          otherSocket.data.interests.includes(interest)
        );
        
        if (commonInterests.length > 0) {
          hasInterestsMatch = true;
          interestsMatchScore = commonInterests.length;
        }
      }
      ...
      console.log(`[Match] Checking ${id}: interestsMatch=${hasInterestsMatch}, interestsScore=${interestsMatchScore}, gender=${genderMatches}, reverseGender=${reverseGenderMatches}`);

      // Perfect match: interests match and both gender preferences are satisfied
      if (hasInterestsMatch && genderMatches && reverseGenderMatches) {
        if (interestsMatchScore > bestMatchScore) {
          bestMatch = otherSocket;
          bestMatchScore = interestsMatchScore;
        }
      }
    ...
    if (selectedMatch) {
      console.log(`[Match] Selected match ${selectedMatch.id} for ${socket.id} with score ${bestMatchScore}`);
    } else {
  ...
  function connectUsers(socketA, socketB, mode) {
    try {
      ...
      const interestsA = socketA.data.interests || [];
      const interestsB = socketB.data.interests || [];
      const matchingInterests = interestsA.filter(interest => interestsB.includes(interest));
      
      console.log(`[Connect] Matching interests:`, matchingInterests);
